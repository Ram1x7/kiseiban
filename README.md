...
3. デプロイしたWorkerの「Settings」→「Variables and Secrets」を開き、以下を追加します
   ...
   **必要に応じて追加する(通常のVariableでOK。未設定でも既定値で動きます):**
   ...
   | `AUTOTRADE_TIMEFRAME_A`〜`_M`(I/K/Lのみ`_HIGH`/`_LOW`) | (未設定) | パターンごとに時間足を個別指定したい場合のみ設定(A/B/C/E/G/H/J/Mは未設定なら`AUTOTRADE_TIMEFRAME`、D/Fは未設定なら`4h`)。パターンの詳細は下記「3.」参照 |
...
⚠️ **パターンGは逆張り(トレンドに逆らう)、それ以外(A/B/D/E/F/H/I/J/K/L/M)はすべて順張り(トレンドに沿う)です。** 両方を同時に有効にすると、同じ時期に正反対の方向へシグナルが出ることがあります。パターンMはダブルトップ/ボトムという「反転」の形を検出しますが、実際のエントリーはネックライン突破の方向についていく形なので、他の順張りパターンと同じ分類にしています。

⚠️ **パターンI/K/Lは他と違い、上位足・下位足の2つの時間足を同時に使います。** パターンごとに時間足を個別に設定できる点は同じですが、I/K/Lはそれぞれ`TIMEFRAME_?_HIGH`(既定`15m`、Lのみ`1h`)・`TIMEFRAME_?_LOW`(既定`1m`、Lのみ`15m`)の2つの変数で個別に設定します(`?`はI・K・Lのいずれか)。他のパターン(A/B/C/E/G/H/J/M)は未設定なら共通の`TIMEFRAME`、D/Fは`4h`が既定値になります。方法A(Worker)なら`AUTOTRADE_TIMEFRAME_A`〜`AUTOTRADE_TIMEFRAME_M`・`AUTOTRADE_TIMEFRAME_I_HIGH`/`_I_LOW`/`_K_HIGH`/`_K_LOW`/`_L_HIGH`/`_L_LOW`、方法B(config.js)なら`AUTOTRADE.TIMEFRAME_A`〜`TIMEFRAME_M`・同様の`_HIGH`/`_LOW`ペアで上書きしてください(`config.example.js`にコメントアウトで記載)。

⚠️ **パターンC/E/H/I/Mは、成立条件を満たした「まさにその瞬間の足」だけで発火するように作られています。** ブレイクした水準を上抜け/下抜けし続けている、フィボナッチのゾーンやFVG/OB帯の中に価格が留まり続けている、RSIが基準の同じ側にあり続けている、といった「状態が続いているだけ」では再度は成立しません。もしこの仕組みがなければ、状態が続く限り毎ティック新規シグナル扱いになり、同時保有上限まで同じ方向へ何度も重ねて発注してしまいます(SLの値幅も近くなりがちなため、小さな押し戻り一つで複数ポジションがまとめて損切りされるリスクがあります)。
