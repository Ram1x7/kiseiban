...
⚠️ **パターンC/E/H/Iは、成立条件を満たした「まさにその瞬間の足」だけで発火するように作られています。** ブレイクした水準を上抜け/下抜けし続けている、フィボナッチのゾーンやFVG/OB帯の中に価格が留まり続けている、RSIが基準の同じ側にあり続けている、といった「状態が続いているだけ」では再度は成立しません。もしこの仕組みがなければ、状態が続く限り毎ティック新規シグナル扱いになり、同時保有上限まで同じ方向へ何度も重ねて発注してしまいます(SLの値幅も近くなりがちなため、小さな押し戻り一つで複数ポジションがまとめて損切りされるリスクがあります)。
