...
3. デプロイしたWorkerの「Settings」→「Variables and Secrets」を開き、以下を追加します
   ...
   **必要に応じて追加する(通常のVariableでOK。未設定でも既定値で動きます):**
   ...
   | `AUTOTRADE_TIMEFRAME_A`〜`_K`(I/Kのみ`_HIGH`/`_LOW`) | (未設定) | パターンごとに時間足を個別指定したい場合のみ設定(A/B/C/E/G/H/Jは未設定なら`AUTOTRADE_TIMEFRAME`、D/Fは未設定なら`4h`)。パターンの詳細は下記「3.」参照 |
...
⚠️ **パターンGは逆張り(トレンドに逆らう)、それ以外(A/B/D/E/F/H/I/J/K)はすべて順張り(トレンドに沿う)です。** 両方を同時に有効にすると、同じ時期に正反対の方向へシグナルが出ることがあります。

⚠️ **パターンI/Kは他と違い、上位足・下位足の2つの時間足を同時に使います。** パターンごとに時間足を個別に設定できる点は同じですが、I/Kはそれぞれ`TIMEFRAME_?_HIGH`(既定`15m`)・`TIMEFRAME_?_LOW`(既定`1m`)の2つの変数で個別に設定します(`?`はIまたはK)。他のパターン(A/B/C/E/G/H/J)は未設定なら共通の`TIMEFRAME`、D/Fは`4h`が既定値になります。方法A(Worker)なら`AUTOTRADE_TIMEFRAME_A`〜`AUTOTRADE_TIMEFRAME_K`・`AUTOTRADE_TIMEFRAME_I_HIGH`/`_I_LOW`/`_K_HIGH`/`_K_LOW`、方法B(config.js)なら`AUTOTRADE.TIMEFRAME_A`〜`TIMEFRAME_K`・同様の`_HIGH`/`_LOW`ペアで上書きしてください(`config.example.js`にコメントアウトで記載)。
