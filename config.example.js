...
const CONFIG = {
  ...
  // 画面上の金額(サイン履歴のTP/SLなど)の先頭に付ける通貨記号。
  ...
  // ---- サイン(シグナル)通知(複数パターンを並行判定) ----
  // 発注は一切行いません。ENABLEDをtrueにしても、画面上の「シグナル監視」
  // トグルを別途ONにしない限りサインは通知されません(二重の安全装置)。
  // パターンの詳細はREADME.mdを参照してください。
  AUTOTRADE: {
    ENABLED: false,          // マスタースイッチ。falseなら画面上のトグルごと無効化される
    SYMBOL: 'USDJPY',        // サイン検知の対象銘柄名(ブローカーにより末尾にサフィックスが付く場合あり。例: USDJPY.)
    ...
    // または配列)。「勝率・期待値を検証」ボタンで期待値がマイナスと
    // 分かったパターンなど。無効化されたパターンはサイン検知・過去の
